...
void Maze::getPathByDFS() {
    ...
    flag[startPoint.Row()][startPoint.Column()] = true;
    Stack<MyPoint> pointStack;
    pointStack.push(startPoint);
    while (!pointStack.isEmpty() && pointStack.getTop() != endPoint) {
        MyPoint adjacentNotVisitedPoinr = getAdjacentNotVisitedPoint(pointStack.getTop());
        if (adjacentNotVisitedPoinr.Row() == -1 || adjacentNotVisitedPoinr.Column() == -1) {
            pointStack.pop();
            continue;
        }
        flag[adjacentNotVisitedPoinr.Row()][adjacentNotVisitedPoinr.Column()] = true;
        pointStack.push(adjacentNotVisitedPoinr);
    }
    pointStack.reverse();
    while (!pointStack.isEmpty()) {
        std::cout<<"("<<pointStack.getTop().Row()<<","<<pointStack.getTop().Column()<<") ";
        pointStack.pop();
    }
}
void Maze::getPathByBFS() {
    if (maze[startPoint.Row()][startPoint.Column()] == WALL || maze[endPoint.Row()][endPoint.Column()] == WALL) {
        std::cout<<"Invalid Start Point or End Point!"<<std::endl;
        return;
    }
    ...
    while (!pointStack.isEmpty()) {
        std::cout<<pointStack.getTop().Row()<<", "<<pointStack.getTop().Column()<<std::endl;
        pointStack.pop();
    }
}
